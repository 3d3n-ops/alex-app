...
// Benchmark Groq vs OpenRouter with server-side tool execution
...
async function main() {
	...
	const userPrompt = 'Find the file where tools are defined and list them.'
...
function printResults(results) {
	...
	for (const provider of Object.keys(grouped)) {
		...
		console.log('  Tool calls (first run):')
		console.log(indent(JSON.stringify(rows[0]?.toolCalls || [], null, 2), 4))
		console.log('')
	}
}
